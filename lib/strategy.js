...
OAuthStrategy.prototype.authenticate = function(req, options) {
  ...
  this._userAuthorizationURL = this.options.userAuthorizationURL;
  this._callbackURL = this.options.callbackURL;
  this._key = this.options.sessionKey || 'oauth';
  this._requestTokenStore =this. options.requestTokenStore || new SessionRequestTokenStore({ key: this._key });
  this._trustProxy = this.options.proxy;
  this._passReqToCallback = this.options.passReqToCallback;
  this._skipUserProfile = (this.options.skipUserProfile === undefined) ? false : this.options.skipUserProfile;
  options = options || {};

  // todo: verify req.session.customer-key & customer-secret for validity.
  this._oauth = new OAuth(this.options.requestTokenURL, this.options.accessTokenURL,
                         req.session.customer_key,  req.session.customer_secret,
                          '1.0', null, this.options.signatureMethod || 'HMAC-SHA1',
                          null, this.options.customHeaders);
